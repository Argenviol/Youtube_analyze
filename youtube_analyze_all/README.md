...
공유**(같은 날 4명 이상의 제목에 같은 단어)로 찾은 뒤, **연속 2일 이상 이어진 것만**
이벤트로 인정합니다. 4명이 하루 같은 게임을 한 것과 8명이 열흘 연속 서버를 돈 것은
성격이 다릅니다 — 앞은 그날 우연히 겹친 쪽에 가깝고, 뒤가 대규모 컨텐츠입니다.
준비·리액션 방송(설명회·클립 월드컵)도 이 기준에서 자동으로 빠집니다.

제목 토큰에서 흔한 말이 걸리는 걸 막으려고 불용어 목록을 쓰는 대신, 토큰이 전체 방송일
중 몇 %에 등장하는지를 세서 **10%를 넘으면 고유명사가 아닌 것으로 봅니다** — 이 기준이
'오늘·같이·게임'을 자동으로 떨어뜨리고 '봉켓몬·갈틱폰·모라하지마'만 남깁니다.

**신의상 공개**는 따로 감지합니다. 제목에 '신의상'이 있고 **평소 조회수의 2배 이상**인
방송만 실제 공개로 봅니다 — 예고·대기방은 0.29~1.60배에 몰려 있고 실제 공개는 2.01배부터
시작해서, 그 사이가 비어 있습니다. 혼자 하루 공개해도 이벤트라 연속일 기준은 적용하지
않습니다.
...
자동 감지는 "같은 날 4명 이상"을 봅니다. 며칠에 걸쳐 멤버가 번갈아 참여하는 기획은
하루만 떼어 보면 1~2명이라 빠집니다(실제로 봉누도가 그렇게 빠졌습니다). 창을 넓혀 보니
재현율은 오르지만 이벤트가 33건→106건으로 늘면서 '오늘'·'광고'·'많이' 같은 말이 이벤트로
올라왔습니다. 그래서 **넓은 창 신호는 `data/event_candidates.csv` 검토 대기열로만** 내보내고,
리포트는 정밀도가 높은 신호와 사람이 확인한 수동 등록만 씁니다.
