...
- **같은 37일에 스텔라이브만 전원이 올랐다.** 구독자 +0명으로 11분의 1 규모의
...
| 하나코 나나 | 215,285 | 23h | 5.3h | 52% | 74% | 3,957 | **+3.79%** |
...
### 같은 기간 성장 (2026-08-12 → 2026-09-18, 37일)
...
커버가 일반 영상 대비 내는 배수는 StelLive 2.05배 · 이세계아이돌 1.95배로 거의 같다 — **커버라는 레버 자체는 그룹을 가리지
...
⚠ **37일·그룹당 6명·1,000 단위 반올림**이다. 홀로라이브 270만 채널은 +999명이
...
멤버별 감성 점수(0.33~0.93)와 최근 21일 치지직 팔로워 성장률의 상관은 **+0.33**(n=10)다. 방향이 있다고 말할 수준이 아니다. 점수 자체도 멤버 간 차이가 거의 없다 — 최저인 멤버도 표본이 야구 영상에 쏠린 탓이지 여론이 나빠서가 아니다.
