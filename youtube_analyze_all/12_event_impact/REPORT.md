...
- 이벤트 구간: 2017-06-01 ~ 2026-09-17 · 총 420건
- 구성: 커버곡 381건, 오리지널곡 18건, 합방 11건, 신의상 4건, 팬펀딩 4건, 콘서트 2건
...
- **대규모 컨텐츠 11건**을 자동 감지했다 — 4명 이상이 **연속 2일 이상**
  함께한 기획만 이벤트로 본다. 마인크래프트 서버·봉켓몬·봉봉팜·팰월드처럼 며칠에 걸친
  기획이 그대로 잡힌다.
- 그 11건 중 **13건이 평소 방송 대비 1.5배 이상**의 조회수를 냈다.
  하루짜리를 섞었을 때는 이 비율이 절반 수준이었다 — **연속 기획만 남기면 효과가 선명해진다.**
  단발 합방과 며칠짜리 기획은 성격이 다른 이벤트다.
- **가장 크게 터지는 이벤트는 합방이 아니라 신의상 공개다.** 조회수는 평소의
  **3.6~6.6배**(합방은 1.5~2.6배), 동시시청자 피크는
  **0.8~16.4배**(같은 기간 나란히 돌던 10인 마인크래프트 합방은
  0.8~2.3배)였다. 사키하네 후야의 첫 신의상은 38,402명 — 평소 피크의 19배다.
- **천장이 가장 높은 건 오리지널곡이다.** MV 가 일반 영상의 7.7~22.4배,
  같은 멤버의 커버곡과 비교해도 1.9~9.1배다. 다만 잴 수 있는 곡이
  한 앨범(4곡)뿐이라 **경향이 아니라 사례**로 읽어야 한다.
- **커버곡은 평소 영상의 4.2배 본다**(같은 기간·성숙 영상만 비교). 콜라보 커버가
  더 잘 되지는 않는다 — 2026년 기준 솔로와 콜라보의 조회수 중앙값이 같다.
- **콘서트는 단독이냐 합동이냐로 갈린다.** 본인 첫 단독 콘서트의 후기 방송은 평소의
  6.0배(안내 쇼츠 1,757,347회)였지만, 같은 멤버가 참여한 그룹 페스티벌은
  1.1~2.7배였다. 합동은 관심이 10명에게 나뉜다.
- **게임사의 공식 푸시는 유저 언급을 만들지 못한다**(상관 -0.01). 희귀도(5성)는
  효과가 있지만 푸시 순서는 없고, 인기는 구작 5성에 쌓여 있다. 스텔라이브 접점은 방송의
  1.9%뿐이며, 게임이 시청자를 끄는 게 아니라 그 멤버의 주력일 때만 평소를 넘는다.
- **같은 36일에 스텔라이브만 전원이 올랐다.** 구독자 +29,000명으로 11분의 1 규모의
  홀로라이브(+40,000명)와 같은 수를 더했고, 이세계아이돌은 전원 감소했다. 커버가 내는
...
- **키리누키는 유일하게 기획할 수 없는 이벤트다.** 클립 표본의 75%가 어떤
  이벤트 키워드에도 안 걸린다 — 클립이 되는 건 기획이 아니라 방송 중 우연한 순간이다.
  개별 클립은 본인 영상의 10% 수준이지만 공식 채널 밖 도달이고,
  상위 3개 채널이 조회수의 32%를 가져간다.
- **동시시청자는 '무엇을 하느냐'보다 '이벤트냐 아니냐'로 갈린다.** 카테고리·시간대·요일을
  바꿔서 얻는 건 최대 3.1배(카테고리)지만, 신의상 공개는 7~19배를 만든다. 관측된
...
|---|---|---|---|---|
| 오리지널곡 MV | 유튜브 조회수 | 일반 영상의 8~22배 | ✅ | 잴 수 있는 곡이 한 앨범 4곡 |
| 신의상 공개 | 치지직 동시시청자 | 평소 피크의 1~16배 (조회수 3.6~6.6배) | ✅ | 연 몇 회짜리 희소 이벤트 |
| 단독 콘서트 | 주변 방송 조회수 | 6.0배 (합동 페스티벌 1.1~2.7배) | ✅ | 구독자 전후 비교 불가(수집 이전) |
| 커버곡 | 유튜브 조회수 | 일반 영상의 4.2배 · 콜라보와 솔로 차이 없음 | ✅ | 멤버별 순위는 표본 1~2곡 |
| 대규모 합방 | 조회수(분량으로) | 1.5~2.6배 · 동시시청자 0.8~2.3배 | ✅ | 연속 2일 이상만 이벤트로 인정 |
| 키리누키 | 공식 채널 밖 도달 | 클립당 본인 영상의 10% | ❌ 순간이 만든다 | 검색 표본, 비율은 상한 |
| 동시시청자 | — | 평소 요인은 최대 3.1배, 이벤트는 7~19배 | — | 2026-08-12 이후만 |
| 댓글 여론 | — | 부정 1.7%, 신호는 주제(사람 > 콘텐츠) | — | 이벤트별 비교 불가 |
| 팬 커머스 | 팬 지출 | 구독 만원 미만 → 팬 광고 수만원 → 굿즈 수십만원 | — | 회사 매출 비공개 |
| DART 재무 | 돈이 남는 곳 | 플랫폼 이익률 18%~27% vs IP 회사 적자 | — | 스텔라이브 DART 미등록 |
| 경쟁사 비교 | 같은 기간 성장 | 스텔라이브 +29,000 vs 홀로 +40,000 (11분의 1 규모) | — | 21일 · 1,000 반올림 |
| 호요버스 | 공식 푸시 → 언급 | 상관 -0.01 (없음) | — | 언급은 인기가 아니라 화제성 |
| 채널 성과 | 유튜브 조회수·성장 | 업로드 빈도 → 조회수 +0.66 · 규모 → 성장 -0.67 | ✅ | n=10 상관 |
| 방송 패턴 | 치지직 VOD·성장 | 시간·심야 → VOD +0.7, 성장은 규모에 딸림 | ✅ | n=10 상관 |
| 시장 | 배경 속도 | 시장 연 10%~16% vs 스텔라이브 46일 +3.3% | — | 한 달 연환산은 거칠다 |
...
| 날짜 | 이벤트 | 참여자 평균 배수 |
|---|---|---|
| 2026-08-31 | 스텔 여름 휴가 신의상 릴레이 공개 (9명) | 6.56배 |
| 2025-07-05 | 신의상 릴레이 공개 (2명) | 4.62배 |
| 2025-09-27 | 세번째 신의상 릴레이 공개 (4명) | 4.05배 |
| 2026-06-21 | 아라하시 타비 신의상 공개 | 3.63배 |
| 2026-06-23 | 마인크래프트 · 봉켓몬 | 2.65배 |
| 2026-08-29 | 마인크래프트 | 2.11배 |
| 2025-12-22 | 마인크래프트 · 멋사 | 2.01배 |
| 2026-05-23 | 마인크래프트 · 모라하지마 | 1.95배 |
| 2025-10-07 | 마인크래프트 | 1.87배 |
| 2026-02-22 | 마인크래프트 | 1.68배 |
...
| 지표 | 신의상 공개 | 대규모 합방 |
|---|---|---|
| VOD 조회수 배수 | **3.6~6.6배** | 1.5~2.6배 |
| 동시시청자 피크 배수 | **0.8~16.4배** | 0.8~2.3배 |

| 멤버 | 공개일 피크 | 평소 피크 | 배수 |
|---|---|---|---|
| 사키하네 후야 | 38,402 | 2,334 | **16.45배** |
| 네네코 마시로 | 28,071 | 3,137 | **8.95배** |
| 아라하시 타비 | 38,237 | 4,483 | **8.53배** |
| 하나코 나나 | 24,385 | 3,253 | **7.50배** |
| 시라유키 히나 | 4,448 | 3,825 | **1.16배** |
| 유즈하 리코 | 3,503 | 3,145 | **1.11배** |
| 아야츠노 유니 | 3,032 | 3,923 | **0.77배** |
...
|---|---|---|---|---|---|---|
| Maid My Way | 아오쿠모 린 | 2026-07-27 | 1,986,358 | 88,779 | **22.4배** | 2.1배 |
| 악당주의보 | 유즈하 리코 | 2026-07-24 | 1,525,252 | 136,423 | **11.2배** | 9.1배 |
| Hush Trap | 하나코 나나 | 2026-07-26 | 1,018,626 | 123,491 | **8.2배** | 1.9배 |
| Berry Verry Strawberry | 텐코 시부키 | 2026-07-25 | 1,131,682 | 146,786 | **7.7배** | 3.5배 |

오리지널곡 MV 는 그 멤버의 일반 영상보다 **7.7~22.4배** 본다. 같은 멤버의
커버곡과 비교해도 **1.9~9.1배**다. 지금까지 잰 이벤트 중 단일 콘텐츠로는
천장이 가장 높다.
...
읽어야 한다. 유즈하 리코의 커버 대비 배수(9.1배)가 유독 큰 것도 곡이 세서가
...
|---|---|---|---|---|
| 아오쿠모 린 | 2곡 | 930,206 | 106,840 | **8.71배** |
| 아라하시 타비 | 1곡 | 810,015 | 111,125 | **7.29배** |
| 시라유키 히나 | 2곡 | 496,466 | 99,332 | **5.00배** |
| 하나코 나나 | 2곡 | 523,438 | 116,492 | **4.49배** |
| 아야츠노 유니 | 1곡 | 395,211 | 92,385 | **4.28배** |
| 사키하네 후야 | 2곡 | 345,987 | 82,029 | **4.22배** |
| 네네코 마시로 | 3곡 | 187,125 | 61,779 | **3.03배** |
| 텐코 시부키 | 1곡 | 324,581 | 146,786 | **2.21배** |
| 아카네 리제 | 1곡 | 251,780 | 211,199 | **1.19배** |
| 유즈하 리코 | 8곡 | 167,568 | 181,727 | **0.92배** |

커버곡은 그 멤버의 일반 영상보다 **중앙값 4.2배** 본다. 10명 중 9명이 2.8~7.4배에
들어간다.

**콜라보 커버가 더 잘 되지는 않는다.** 2026년 커버만 놓고 보면 솔로 395,211회 ·
콜라보 386,796회로 차이가 없다. 전체 기간으로는 콜라보가 높아 보이는데, 콜라보 커버가
**2024년부터만 존재해서** 생기는 착시다(그 이전 카탈로그는 전부 솔로).

### 읽을 때 주의

유즈하 리코만 0.92배로 유일하게 1배 미만이고, 동시에 커버를 가장 자주 올린다
...
|---|---|---|---|---|---|
| 2026-07-11 아카네 리제 첫 단독 콘서트 | 아카네 리제 | 4건 | 13,753 | 2,309 | **5.96배** |
| 2025-12-20 2025 THE 1ST STELLIVE  | 시라유키 히나 | 2건 | 11,462 | 4,273 | **2.68배** |
| 2025-12-20 2025 THE 1ST STELLIVE  | 사키하네 후야 | 1건 | 3,551 | 2,726 | **1.30배** |
| 2025-12-20 2025 THE 1ST STELLIVE  | 유즈하 리코 | 1건 | 5,055 | 4,192 | **1.21배** |
| 2025-12-20 2025 THE 1ST STELLIVE  | 아카네 리제 | 7건 | 3,942 | 3,278 | **1.20배** |
| 2025-12-20 2025 THE 1ST STELLIVE  | 하나코 나나 | 1건 | 3,975 | 3,794 | **1.05배** |

**본인 단독 콘서트가 그룹 합동보다 훨씬 크게 남는다.** 아카네 리제의 첫 단독 콘서트
후기 방송은 평소의 6.0배였고, 콘서트 안내 쇼츠는 **1,757,347회**로 그 채널
...
| 멤버 | 구독자 | 평균 조회수 | 주당 업로드 | 쇼츠 | 참여율 | 성장 |
|---|---|---|---|---|---|---|
| 사키하네 후야 | 135,000 | 116,848 | 3.5 | 22% | 4.0% | **+3.85%** |
| 유즈하 리코 | 222,000 | 292,449 | 5.6 | 42% | 3.6% | **+3.74%** |
| 하나코 나나 | 207,000 | 190,850 | 6.2 | 48% | 4.1% | **+2.99%** |
| 아오쿠모 린 | 284,000 | 197,567 | 4.2 | 22% | 3.4% | **+2.90%** |
| 아라하시 타비 | 262,000 | 212,714 | 4.0 | 18% | 3.1% | **+2.75%** |
| 텐코 시부키 | 230,000 | 209,664 | 6.2 | 46% | 4.0% | **+2.68%** |
| 아카네 리제 | 356,000 | 312,681 | 5.1 | 42% | 3.2% | **+2.59%** |
| 네네코 마시로 | 184,000 | 118,931 | 2.0 | 8% | 4.4% | **+2.22%** |
| 아야츠노 유니 | 361,000 | 137,239 | 2.9 | 16% | 3.4% | **+0.84%** |
| 시라유키 히나 | 320,000 | 188,109 | 3.5 | 38% | 3.4% | **+0.31%** |

| 요인 | → 평균 조회수 | → 36일 구독자 성장 |
|---|---|---|
| 주당 업로드 | +0.66 | +0.45 |
| 쇼츠 비중 | +0.65 | +0.15 |
| 참여율 | -0.52 | +0.28 |
...
**업로드 빈도와 평균 조회수의 상관 +0.66** — 이 규모대에서는 물량이 조회수를
견인한다(01의 결론 그대로). 쇼츠 비중도 같은 방향이다. 반면 **구독자 규모와 성장률은
-0.67** — 이미 큰 채널일수록 느리게 큰다. 참여율은 조회수와 음(−), 성장과 양(+)이라
...
| 멤버 | 팔로워 | 주당 시간 | 평균 길이 | 심야 | 게임 | VOD 조회수 | 성장 |
|---|---|---|---|---|---|---|---|
| 사키하네 후야 | 165,104 | 26h | 5.0h | 38% | 31% | 3,566 | **+5.33%** |
| 유즈하 리코 | 217,594 | 34h | 7.0h | 67% | 38% | 4,507 | **+3.94%** |
| 하나코 나나 | 215,285 | 23h | 5.3h | 52% | 74% | 3,957 | **+3.78%** |
| 아오쿠모 린 | 227,086 | 21h | 4.7h | 65% | 87% | 3,971 | **+3.64%** |
| 아카네 리제 | 260,967 | 23h | 5.0h | 65% | 52% | 3,732 | **+3.19%** |
| 네네코 마시로 | 216,495 | 25h | 4.8h | 22% | 42% | 3,338 | **+3.18%** |
| 아라하시 타비 | 263,119 | 39h | 7.4h | 66% | 51% | 5,565 | **+3.05%** |
| 텐코 시부키 | 247,829 | 36h | 7.1h | 81% | 71% | 6,824 | **+2.78%** |
| 아야츠노 유니 | 307,614 | 36h | 6.4h | 46% | 26% | 3,889 | **+1.88%** |
| 시라유키 히나 | 240,850 | 36h | 6.1h | 71% | 39% | 5,859 | **+1.69%** |

| 요인 | → 평균 VOD 조회수 | → 36일 팔로워 성장 |
|---|---|---|
| 주당 방송 시간 | +0.71 | -0.53 |
| 평균 방송 길이 | +0.75 | -0.37 |
| 심야 비중 | +0.76 | -0.30 |
...
방송을 많이 해서 덜 크는 게 아니라, **팔로워 규모 → 성장 상관이 -0.82**로 압도적이라
...
| 출처 | 기간 | 시장 규모 | 연평균 성장률 |
|---|---|---|---|
| SkyQuest Vtuber Market Report | 2026→2031 | $3.13B → $4.94B | **+9.6%** |
| Global Growth Insights | 2026→2032 | $3.31B → $8.24B | **+16.4%** |

스텔라이브 탤런트 10명의 유튜브 구독자 합계는 2026-08-03 2,480,000 → 2026-09-18 2,561,000
(**+3.27%**, 46일)이다. 단순 연환산하면 +29%로 시장 성장률의 몇 배지만,
**한 달을 연환산한 숫자**이고 그 달에 cliché EP 발매가 있었다. 시장보다 빠르게 크고
있다는 방향까지만 읽어야 한다.

| 시장 지표 | 값 | 연도 | 출처 |
|---|---|---|---|
| 구독+후원 매출 비중 | 53% | 2025 | SkyQuest Vtuber Market Report |
| 유튜브 매출 비중 | 50% | 2025 | SkyQuest Vtuber Market Report |
| 아시아태평양 지역 점유율 | 65% | 2025 | SkyQuest Vtuber Market Report |
| 치지직 월간활성이용자(MAU) | 2,420,000 | 2026 | 파이낸셜뉴스 |

시장 매출의 절반은 구독·후원(치지직 쪽), 절반은 유튜브다. 앞 절들이 찾은 "신의상은
치지직을, 곡은 유튜브를 움직인다"는 분리가 시장 매출 구조와 같은 모양이다. 치지직
MAU 2,420,000명 기준으로 사키하네 후야의 신의상 피크 38,402명은 플랫폼 전체 월간
이용자의 1.6%가 한 방송에 모인 순간이다.

**스텔라이브 연표** — 2014-11 강지 개인 스트리머로 유튜브 채널 개설 · 2021-01 강지 버추얼 크리에이터 기획사 스텔라이브 설립 준비 시작 · 2022-12 1세대 EVERYS 유닛 데뷔 (아야츠노 유니 등) · 2023-06 2세대 UNIVERSE 유닛 데뷔 (네네코 마시로 등) · 2024-01 3세대 cliché 유닛 데뷔 · 2025-07 브레이브 그룹(Brave Group)에 인수합병 발표 · 2025-09 브레이브 그룹 투자 라운드 참여 · 2025-12 첫 단체 콘서트 "2025 THE 1ST STELLIVE FESTIVAL [STAR TRAIL]" 개최
...
매칭 가능한 캐릭터 203명 중 61%만 리뷰에 한 번이라도 언급된다.
공식 푸시 순위(5성·최신순)와 언급률의 상관은 **-0.01** — 없다. 출시 최신순과의
상관도 +0.12로 약하다.
...
| 게임 | 캐릭터 | 출시 | 언급 | 푸시 순위 | 감성(게임 평균 대비) |
|---|---|---|---|---|---|
| 붕괴:스타레일 | 반디 | 2024-06-19 | **70** | 62위 | +0.32 |
| 붕괴:스타레일 | 아케론 | 2024-03-27 | **25** | 68위 | +0.32 |
| 원신 | 푸리나 | 2023-11-06 | **20** | 78위 | +0.92 |
| 원신 | 마비카 | 2024-12-30 | **18** | 46위 | -0.63 |
| 붕괴:스타레일 | 은랑 | 2023-06-07 | **18** | 89위 | +0.67 |
| 붕괴:스타레일 | 키레네 | 2025-11-05 | **18** | 22위 | -1.05 |
| 붕괴:스타레일 | 파이논 | 2025-07-02 | **16** | 33위 | -0.41 |
| 붕괴:스타레일 | 블레이드 | 2023-07-19 | **14** | 87위 | +0.71 |

언급 상위 8명의 푸시 순위 중앙값은 **65위**(낮을수록 강한 푸시)이고 출시는
대부분 1~2년 전이다. 반대로 지금 가장 세게 미는 5명의 언급 중앙값은 **0건**
(상위 8명은 18건)이며 감성은 게임 평균보다 낮은 쪽이 많다. **인기는 구작
5성에 쌓여 있고, 신작 푸시는 언급을 만들지 못한 채 불만만 만든다.**

### ② 스텔라이브 접점 — 방송 56건 / 2968건 (1.9%)

| 멤버 | 게임 | 방송 | 호요 방송 중앙 | 평소 중앙 | 배수 | [광고] | 광고 배수 | 비광고 배수 |
|---|---|---|---|---|---|---|---|---|
| 아카네 리제 | 붕괴: 스타레일 | 21회 | 5,794 | 2,930 | **1.98배** | 3회 | 1.12배 | 2.12배 |
| 하나코 나나 | 붕괴: 스타레일 | 17회 | 4,214 | 3,148 | **1.34배** | 5회 | 1.10배 | 1.40배 |
| 유즈하 리코 | 젠레스 존 제로 | 8회 | 4,381 | 3,961 | **1.11배** | 3회 | 0.91배 | 1.15배 |
| 사키하네 후야 | 젠레스 존 제로 | 7회 | 1,944 | 2,268 | **0.86배** | 1회 | 1.42배 | 0.77배 |

방송 3회 이상인 멤버 4명 중 **아카네 리제만 평소보다 확실히 더 본다**(1.98배, 붕괴: 스타레일 21회). 나머지는 하나코 나나 1.34배·유즈하 리코 1.11배·사키하네 후야 0.86배로 평소와 같거나 낮다. 게임 자체가 시청자를 끌지는 않고, 그 멤버의 주력 콘텐츠일 때만 세다. 또 하나 — **[광고] 방송은 같은 게임의 비광고 방송보다 덜 본다.** 광고 2회 이상인 3명 중 3명이 그렇고(아카네 리제 1.12배 vs 2.12배·하나코 나나 1.10배 vs 1.40배·유즈하 리코 0.91배 vs 1.15배), 광고 방송은 게임을 '하는' 게 아니라 '보여주는' 방송이라 시청자가 덜 붙는 것으로 읽힌다.

⚠ **언급량은 인기가 아니라 화제성이다.** 리뷰는 불만을 쓰러 오는 곳이라 언급이 많은
캐릭터가 사랑받는 캐릭터가 아닐 수 있다(감성 열이 그걸 보여준다). Google Trends 는
매 호출 429 로 실패해 설계에서 뺐다(10 참고). 멤버별 배수는 3회 미만이면
...
### 같은 기간 성장 (2026-08-12 → 2026-09-17, 36일)

| 그룹 | 구독자 합계 | 증감 | 성장률 | 멤버 중앙값 | 오른 멤버 | 내린 멤버 |
|---|---|---|---|---|---|---|
| StelLive | 1,618,000 | **+29,000** | **+1.83%** | +2.41% | 6/6 | 0/6 |
| 홀로라이브 | 18,070,000 | **+40,000** | **+0.22%** | +0.00% | 2/6 | 1/6 |
| 이세계아이돌 | 2,472,000 | **-10,000** | **-0.40%** | -0.42% | 0/6 | 6/6 |
...
|---|---|---|---|
| StelLive | 3.9편 | **8.0%** (24편) | 2.05배 |
| 홀로라이브 | 5.0편 | **3.3%** (26편) | 5.40배 |
| 이세계아이돌 | 3.5편 | **0.6%** (1편) | 1.95배 |

스텔라이브는 세 그룹 중 **업로드는 가장 적게 하면서 커버 비중은 가장 높다.** 그리고
커버가 일반 영상 대비 내는 배수는 StelLive 2.05배 · 홀로라이브 5.40배 · 이세계아이돌 1.95배로 거의 같다 — **커버라는 레버 자체는 그룹을 가리지
...
|---|---|---|---|
| StelLive | 246,000 | **0.82** | 3.6% |
| 홀로라이브 | 884,000 | **0.19** | 6.5% |
| 이세계아이돌 | 415,000 | **0.23** | 5.1% |
...
⚠ **36일·그룹당 6명·1,000 단위 반올림**이다. 홀로라이브 270만 채널은 +999명이
늘어도 0으로 보이므로 상대 성장률 비교는 스텔라이브에 유리하다. 절대 증가로는
스텔라이브 +29,000명 · 홀로라이브 +40,000명으로 **11분의 1 규모에서 같은
수를 더했다**가 정확한 진술이다.
...
멤버별 감성 점수(0.33~0.93)와 최근 22일 치지직 팔로워 성장률의 상관은 **+0.34**(n=10)다. 방향이 있다고 말할 수준이 아니다. 점수 자체도 멤버 간 차이가 거의 없다 — 최저인 멤버도 표본이 야구 영상에 쏠린 탓이지 여론이 나빠서가 아니다.
...
|---|---|---|---|---|---|---|
| 텐코 시부키 | 22편 | 5,517,479 | 142,898 | 158,535 | 0.90배 | 13개 |
| 아야츠노 유니 | 41편 | 5,361,643 | 14,330 | 99,778 | 0.14배 | 20개 |
| 아카네 리제 | 22편 | 2,663,348 | 18,787 | 228,742 | 0.08배 | 8개 |
| 하나코 나나 | 27편 | 1,778,589 | 21,496 | 128,049 | 0.17배 | 17개 |
| 네네코 마시로 | 39편 | 1,679,023 | 4,883 | 74,538 | 0.07배 | 12개 |
| 아라하시 타비 | 47편 | 1,661,083 | 11,150 | 119,149 | 0.09배 | 13개 |
| 시라유키 히나 | 29편 | 1,080,608 | 6,588 | 131,203 | 0.05배 | 14개 |
| 유즈하 리코 | 17편 | 800,420 | 18,410 | 175,701 | 0.10배 | 11개 |
| 아오쿠모 린 | 7편 | 413,091 | 33,290 | 110,493 | 0.30배 | 4개 |
| 사키하네 후야 | 27편 | 85,462 | 2,014 | 83,157 | 0.02배 | 4개 |

**클립 하나는 작지만 수가 많고, 공식 채널 밖에 있다.** 클립 조회수 중앙값은 그 멤버
영상의 **10%** 수준이다(멤버별로 2%~67%로 편차가 크다 — 사키하네 후야가
0.02배로 최저인 건 2025-09 데뷔라 클립 생태계가 아직 얇기 때문이고, 텐코 시부키가
0.67배로 최고인 건 역대 1위 클립을 포함한 소수의 큰 클립이 끌어올린 결과다). 대신 멤버당 7~47편이 76개 채널에서
...
| 주제 | 클립 수 | 비중 | 조회수 중앙값 |
|---|---|---|---|
| 커버·노래 | 64편 | 23.0% | 34,216 |
| 합방·서버 | 4편 | 1.4% | 10,572 |
| 게임 | 2편 | 0.7% | 22,450 |
| 신의상 | 1편 | 0.4% | 12,170 |
| **어느 것도 아님** | — | **75.2%** | — |

여기가 다른 이벤트와 갈리는 지점이다. 표본 278건 중 **75%가 어떤
이벤트 키워드에도 걸리지 않는다.** 상위 클립 제목이 「말 실수한 유니」·「하이요 얼굴
공개 ㄷㄷ」·「힘드신군뇨」처럼 **방송 중 우연한 순간**이다. 커버·노래만 예외적으로
23%를 차지하고, 실제로 역대 1위 클립도 커버 라이브다.
...
클립 278건이 76개 채널에서 나오는데, **상위 3개 채널이 조회수의
32%, 상위 10개가 68%**를 가져간다. 2차창작이 넓게 퍼져 있다기보다
...
관측된 방송 세션 938건의 피크 분포는 심하게 치우쳐 있다 — 중앙값 **3,598명**, 상위 5% 7,442명, 상위 1% 10,322명, 최대 **38,402명**. 평균 근처에 몰려 있는 게 아니라 꼬리가 길다.
...
| 5 | 09-05 | 아라하시 타비 | **16,242** | ✨한강 불꽃놀이 함께 봐요 (ﾉ*ФωФ)ﾉ✨ |

**평소 요인은 이벤트에 비하면 미미하다.** 세션 5건 이상인 구간만 놓고 보면 —

| 요인 | 중앙값 범위 | 최대/최소 |
|---|---|---|
| 카테고리 | 2,256 ~ 6,936명 (음악/노래가 최고) | 3.1배 |
| 시작 시간대 | 3,514 ~ 3,875명 | 1.1배 |
| 요일 | 3,044 ~ 3,969명 | 1.3배 |

가장 큰 카테고리조차 3.1배다. 반면 신의상 공개는 같은 멤버의 평소 피크 대비
...
|---|---|---|---|---|---|
| 2026-08-31 | 스텔 여름 휴가 신의상 릴레이 공개 (9명) | 사키하네 후야 | 38,402 | 2,334 | 16.45배 |
| 2026-08-31 | 스텔 여름 휴가 신의상 릴레이 공개 (9명) | 네네코 마시로 | 28,071 | 3,137 | 8.95배 |
| 2026-08-31 | 스텔 여름 휴가 신의상 릴레이 공개 (9명) | 아라하시 타비 | 38,237 | 4,483 | 8.53배 |
| 2026-08-31 | 스텔 여름 휴가 신의상 릴레이 공개 (9명) | 하나코 나나 | 24,385 | 3,253 | 7.50배 |
| 2026-08-29 | 마인크래프트 | 네네코 마시로 | 7,068 | 3,076 | 2.30배 |
| 2026-08-29 | 마인크래프트 | 사키하네 후야 | 4,734 | 2,267 | 2.09배 |
| 2026-09-15 | Grand Theft Auto V · 하루 | 아카네 리제 | 6,328 | 3,340 | 1.89배 |
...
|---|---|---|---|---|---|---|
| 2026-08-29 | 콧치노 켄토 - 네, 기꺼이 [はいよろこんで / | 네네코 마시로 | 치지직 팔로워 | +68 | +477 | +602.1% |
| 2026-09-01 | 아틀란티스 소녀 (Atlantis Princes | 시라유키 히나 | 치지직 팔로워 | +28 | +126 | +349.8% |
| 2026-08-31 | 스텔 여름 휴가 신의상 릴레이 공개 (9명) | 유즈하 리코 | 유튜브 구독자 | +83 | +286 | +242.9% |
| 2026-08-29 | 마인크래프트 | 텐코 시부키 | 치지직 팔로워 | +55 | +167 | +200.6% |
| 2026-08-31 | 푸른 산호초(青い珊瑚礁) - 松田 聖子 l 사키 | 사키하네 후야 | 치지직 팔로워 | +123 | +308 | +149.9% |
| 2026-08-29 | 마인크래프트 | 아카네 리제 | 치지직 팔로워 | +62 | +153 | +146.8% |
| 2026-08-29 | 마인크래프트 | 아야츠노 유니 | 치지직 팔로워 | +58 | +113 | +95.2% |
| 2026-08-22 | [4K] 숨바꼭질 かくれんぼ - AliA / 유 | 유즈하 리코 | 치지직 팔로워 | +97 | +182 | +87.0% |
| 2026-08-22 | [4K] 숨바꼭질 かくれんぼ - AliA / 유 | 유즈하 리코 | 유튜브 구독자 | +143 | +250 | +75.0% |
| 2026-08-29 | 마인크래프트 | 네네코 마시로 | 치지직 팔로워 | +68 | +119 | +74.8% |
