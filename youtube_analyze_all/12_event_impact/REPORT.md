# 프로젝트 12 · StelLive 이벤트 임팩트 분석

- 기준일: 2026-08-29
- 데이터 소스: 01·02·03·08 의 기존 수집분 재사용 (신규 API 호출 없음)
- 이벤트 구간: 2017-06-01 ~ 2026-08-29 · 총 409건
- 구성: 커버곡 365건, 합방 35건, 오리지널곡 7건, 콘서트 2건

## 이 프로젝트가 따로 있는 이유

다른 프로젝트는 "지금 얼마인가"를 답한다. 이 프로젝트는 **"무슨 일이 있어서 그렇게
됐는가"**를 답한다. 대규모 합방·커버곡 발매·콘서트 같은 사건을 자동으로 찾아 기억하고,
그 전후로 팔로워·구독자·동시시청자가 어떻게 움직였는지 붙인다.

이벤트는 손으로 적어 두지 않으면 잊힌다. 그래서 치지직 VOD 의 제목과 카테고리에서
합방을 자동 감지하고(`collect.py`), 처음 감지한 날짜를 `data/events_seen.csv` 에
append-only 로 남긴다.

## 핵심 요약

- 관측 구간 전체에서 **대규모 합방(4명 이상) 35건**을 자동 감지했다.
  마인크래프트 서버·봉켓몬·팰월드·갈틱폰처럼 이름이 붙은 이벤트가 그대로 잡힌다.
- 소급 측정한 합방 중 **17건이 평소 방송 대비 1.5배 이상**의 조회수를 냈다.
  합방은 평균적으로 그 멤버의 평소 방송보다 크게 터진다.
- 전후 비교(팔로워·구독자·동시시청자)는 **일일 수집이 시작된 2026-08-12 이후 이벤트만**
  가능하다. 그 이전은 기준선이 없어 계산하지 않는다 — 0%가 아니라 측정 불가다.
- 최근 4건은 **관측 중**이다. 이벤트 후 5일이 지나야 전후 비교를 낸다 —
  창이 안 찬 상태로 계산하면 관측 부족이 지표 폭락으로 둔갑한다.
- **수익은 어떤 방법으로도 잴 수 없다.** 치지직 후원·구독 수익도 유튜브 광고 수익도
  공개되지 않는다. 대리 지표로 노출량(동시시청자 피크·조회수)만 낸다.

## 합방 효과 — 이벤트 방송 조회수 ÷ 평소 방송 (소급 가능)

| 날짜 | 이벤트 | 참여자 평균 배수 |
|---|---|---|
| 2026-08-15 | 봉누도2 (RP 서버) — 설명회·준비 구간 | 19.91배 |
| 2025-12-22 | 마인크래프트 | 2.92배 |
| 2026-06-23 | 마인크래프트 · 봉켓몬 | 2.65배 |
| 2025-10-07 | 마인크래프트 | 2.27배 |
| 2026-01-01 | 같이 | 2.23배 |
| 2026-06-12 | 2026 FIFA 북중미 월드컵 | 2.20배 |
| 2026-07-13 | 팰월드 | 2.15배 |
| 2026-05-23 | 마인크래프트 · 모라하지마 | 1.95배 |
| 2026-02-22 | 마인크래프트 | 1.89배 |
| 2026-02-22 | 오픈런 | 1.79배 |

## 동시시청자 피크 (2026-08-12 이후)

| 날짜 | 이벤트 | 멤버 | 이벤트 피크 | 평소 피크 | 배수 |
|---|---|---|---|---|---|
| 2026-08-15 | 봉누도2 (RP 서버) — 설명회·준비 구간 | 하나코 나나 | 9,915 | 3,221 | 3.08배 |
| 2026-08-15 | 봉누도2 (RP 서버) — 설명회·준비 구간 | 시라유키 히나 | 9,749 | 3,172 | 3.07배 |
| 2026-08-15 | 봉누도2 (RP 서버) — 설명회·준비 구간 | 아야츠노 유니 | 8,655 | 4,068 | 2.13배 |
| 2026-08-15 | 봉누도2 (RP 서버) — 설명회·준비 구간 | 텐코 시부키 | 9,910 | 5,060 | 1.96배 |
| 2026-08-22 | [4K] 숨바꼭질 かくれんぼ - AliA / 유 | 유즈하 리코 | 4,882 | 3,489 | 1.40배 |
| 2026-08-16 | 데드 바이 데이라이트 | 아오쿠모 린 | 3,594 | 3,472 | 1.03배 |
| 2026-08-16 | 데드 바이 데이라이트 | 시라유키 히나 | 3,254 | 3,315 | 0.98배 |
| 2026-08-27 | 이터널 리턴 | 하나코 나나 | 3,141 | 3,547 | 0.89배 |

## 팔로워·구독자 전후 변화 (2026-08-12 이후)

| 날짜 | 이벤트 | 멤버 | 지표 | 이전(일평균) | 이후(일평균) | 변화 |
|---|---|---|---|---|---|---|
| 2026-08-22 | [4K] 숨바꼭질 かくれんぼ - AliA / 유 | 유즈하 리코 | 유튜브 구독자 | +143 | +300 | +110.0% |
| 2026-08-22 | [4K] 숨바꼭질 かくれんぼ - AliA / 유 | 유즈하 리코 | 치지직 팔로워 | +97 | +152 | +56.7% |

## 데이터 함정

- **합방 자동 감지는 치지직 방송이 있어야 한다.** 유튜브에서만 진행한 합방이나
  오프라인 행사는 잡히지 않는다. `data/events_manual.csv` 에 손으로 적어야 한다.
- **오리지널곡은 자동 감지되지 않는다.** 02는 커버곡만 수집하므로, 오리지널 발매는
  수동 등록 대상이다.
- **유튜브 구독자는 1,000 단위 반올림**이라 이벤트 다음날 변화가 계단으로만 보인다.
  7일 창으로만 낸 이유다. 짧은 창은 치지직 팔로워(정확한 정수)를 볼 것.
- **조회수 배수는 시간이 지날수록 커진다.** VOD 조회수는 누적이라 오래된 이벤트가
  유리하다. 같은 시기 이벤트끼리 비교할 것.

## 산출물

- `data/events.csv` — 감지·등록된 전체 이벤트
- `data/events_seen.csv` — 처음 감지한 날짜 (append-only 기억)
- `data/events_manual.csv` — 손으로 등록하는 이벤트 (콘서트·오리지널곡·오프라인)
- `data/event_vod_multiple.csv` · `event_impact.csv` · `event_ccu.csv`
- `charts/` · `sql/events.db` · `site/index.html`
