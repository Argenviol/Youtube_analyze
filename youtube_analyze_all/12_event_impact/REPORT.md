...
- **게임사의 공식 푸시는 유저 언급을 만들지 못한다**(상관 -0.16). 희귀도(5성)는
...
| 호요버스 | 공식 푸시 → 언급 | 상관 -0.16 (없음) | — | 언급은 인기가 아니라 화제성 |
...
매칭 가능한 캐릭터 301명 중 41%만 리뷰에 한 번이라도 언급된다.
공식 푸시 순위(5성·최신순)와 언급률의 상관은 게임별로 **붕괴3rd -0.27 · 붕괴:스타레일 -0.09 · 원신 -0.30 · 젠레스 존 제로 +0.01** (평균 -0.16) — 없다.
출시 최신순과의 상관도 평균 +0.21로 약하다. 순위는 게임 안에서만 매긴 값이라
