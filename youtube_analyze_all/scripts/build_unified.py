...
GAME_FINDINGS = [
    ("미는 캐릭터와 반응하는<br>캐릭터가 다르다",
     "게임마다 따로 봐도 공식 푸시 1위(최신 5성)와 리뷰 언급량 1위가 일치하지 않는다. "
     "출시 순서는 회사가 정하지만 화제는 그대로 따라오지 않는다."),
    ("게임은 섞지 않는다,<br>기간은 맞춘다",
     "다섯 게임은 출시 주기·캐릭터 풀·표본이 달라 순위와 격차를 게임 안에서만 매긴다. "
     "대신 수집 기간은 같은 창(최근 1년)으로 맞춰, 게임마다 다른 건수는 그 게임의 사실로 남긴다."),
    ("언급량은 인기가 아니라<br>화제성이다",
     "매칭 가능한 캐릭터의 3분의 1 이상이 수집된 리뷰에 한 번도 등장하지 않는다. "
     "언급 0을 비인기로 읽으면 안 된다 — 표본이 닿지 않은 것이다."),
    ("못 쓴 지표도<br>기록한다",
     "원래 쓰려던 검색 관심도(Google Trends)는 아직 분석에 없다. 실행마다 실제로 호출해 "
     "성공·실패를 상태 파일로 남기고, 붕괴3rd 한글 이름이 수동 표라는 것도 리포트에 그대로 적는다."),
]
...
REPORTS = [
    ...
    dict(
        ...
        digest_headline="다섯 게임 캐릭터 인기도,<br>측정된 숫자만 모은 요약판",
        lead="원신·붕괴:스타레일·젠레스 존 제로·붕괴3rd·블루 아카이브 다섯 게임의 캐릭터 마스터, 한국 스토어 리뷰, "
             "공식 한국 유튜브 채널 댓글만으로 <strong>공식 푸시</strong>와 <strong>유저 반응</strong>이 어디서 갈리는지 "
             "<strong>게임별로 따로</strong> 봅니다. 수집 기간은 다섯 게임에 같은 창(최근 1년)을 씁니다. "
