...
PROJECTS = [
    ...
    ("12_event_impact", 12, "이벤트 임팩트", True,
     ...
     "**게임사의 공식 푸시는 유저 언급을 만들지 못한다**(상관 -0.01) — 희귀도는 효과가 있지만 푸시 순서는 "
     "없고 인기는 구작 5성에 쌓여 있다. 스텔라이브 접점은 방송의 2%뿐이며 그 멤버의 주력일 때만 평소를 넘는다. "
     "**같은 21일에 스텔라이브만 전원이 올랐다** — 11분의 1 규모에서 홀로라이브와 같은 수의 구독자를 "
     "더했고 이세계아이돌은 전원 감소했다. 커버의 배수는 그룹이 같은데(2.0~2.1배) 스텔라이브가 "
     "커버를 가장 자주 낸다. 다만 참여율은 셋 중 최저다 — 도달은 넓고 참여는 얕다. "
     "**팬덤의 돈은 플랫폼에 쌓이고 IP 회사엔 남지 않는다** — 스텔라이브는 DART 에 없어 못 재지만, "
     "플랫폼(NAVER·SOOP)은 10년 연속 흑자에 이익률 18~27%, IP 회사(샌드박스)는 8년 연속 적자다. "
     "**팬은 자기 돈으로 광고를 낸다** — 팬 제작 생일·기념 광고 펀딩 4건이 목표의 3~5배를 모았고 "
     "1인당 3~4만원이다. 지출은 정기 구독(월 7천원) → 팬 광고(수만원) → 공식 굿즈(수십만원)로 "
     "목적에 따라 자릿수가 바뀌는데, 동종업계 감사 재무는 연속 영업적자다. "
     "**댓글 여론은 긍정/부정으로 가를 수 없다** — 긍정 80%·부정 1.7%이고 부정 5건은 전부 야구팀 "
     "얘기라 멤버를 향한 부정은 0건이다. 신호는 주제에 있다: 성격·개그 댓글이 가장 많고 가장 "
     "긍정적이고 좋아요도 방송내용·게임의 1.9배다 — 팬은 콘텐츠가 아니라 사람에 반응한다. "
     "**키리누키만은 기획할 수 없다** — 클립 표본의 77%가 어떤 이벤트 키워드에도 안 걸리고, "
     "상위 클립은 「말 실수한 유니」 같은 방송 중 우연한 순간이다. 개별 클립은 본인 영상의 12% "
     "수준이지만 공식 채널 밖 도달이고, 상위 3개 채널이 클립 조회수의 33%를 가져간다. "
     "**동시시청자는 '무엇을 하느냐'보다 '이벤트냐 아니냐'로 갈린다** — 카테고리를 바꿔서 얻는 건 "
     "2.1배, 시간대·요일은 1.2~1.5배인데 신의상 공개는 7~19배다. 관측된 세션 344건 중 상위 4개가 "
     "전부 신의상이고 4위와 5위 사이에만 2.4배 차이가 난다. "
     ...
     "천장이 가장 높은 건 **오리지널곡**이다 — MV 가 일반 영상의 6~18배, 같은 멤버의 커버곡과 "
     "비교해도 2~3배다(단 잴 수 있는 곡이 한 앨범 4곡뿐이라 경향이 아니라 사례다). "
     "커버곡은 같은 기간 일반 영상의 **중앙값 4배**를 보고, 콜라보 커버가 더 잘 되지는 않는다"
     "(2026년 솔로·콜라보 조회수 중앙값이 같다). "
