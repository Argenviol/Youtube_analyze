...
PROJECTS = [
    ...
    ("10_hoyoverse", 10, "게임 캐릭터 인기도", False,
     "게임사가 미는 캐릭터와 유저가 반응하는 캐릭터가 **다르다.** 원신·붕괴:스타레일·젠레스 존 제로·"
     "붕괴3rd·블루 아카이브를 각각 따로 봐도 공식 푸시 1위(최신 최고 등급)와 언급량(공식 채널 댓글+리뷰) 1위가 일치하지 않고, "
     "반응 상위에는 출시가 오래된 캐릭터가 꾸준히 올라온다. 순위·격차는 **게임 안에서만** 매기고 "
     "**리뷰 기간만 같은 창(최근 1년)** 으로 맞춘다 — 건수는 게임마다 다르고, 붕괴3rd 는 한 해 "
     "리뷰가 두 자릿수라 언급 순위가 사실상 서지 않는다는 것까지 결과다. 매칭 가능한 캐릭터의 "
     "**3분의 1 이상**이 리뷰에 한 번도 언급되지 않아, 언급량은 '인기'가 아니라 **'화제성'** 의 "
     "근사치로만 읽어야 한다. Google Trends 는 실행마다 실제로 호출하고 결과를 상태 파일로 남긴다."),
    ...
    ("12_event_impact", 12, "이벤트 임팩트", True,
     "**이벤트가 지표를 움직이고, 평소 요인은 오차 범위다.** 카테고리·시간대·요일을 바꿔 얻는 건 "
     "최대 2배인데, 신의상 공개는 동시시청자를 **7~19배**, 오리지널곡 MV 는 조회수를 **6~18배** 만든다. "
